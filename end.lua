...
function end_draw(x)
	...
	-- bird
	if x == 0 then
		love.graphics.drawq(image,quadbird2, posX, 470, 0.2)
	else 
		posY = posY + 40
		if posY <= 470 then
			love.graphics.drawq(image,quadbird2, posX, posY, 0.2)
		else
			love.graphics.drawq(image,quadbird2, posX, 470, 0.2)
		end
	end
	-- game over
	love.graphics.drawq(image,quadgaov,gamewidth/2-508/6,gameheight/4)


	-- board
	love.graphics.drawq(image,quadbase,gamewidth/2-590/6,gameheight/3)

	if score > best then
		-- best = score
		love.graphics.drawq(image,quadnew, 250, gameheight/3 + 70)
	end
	...
	sc = score
	if sc == 0 then
		love.graphics.drawq(image,quad_num[0], 230,gameheight/3 + 28, 0, 0.6, 0.6)
	end
	while sc ~= 0 do
		love.graphics.drawq(image,quad_num[sc % 10], 230 + len * 12, gameheight/3 + 28, 0 , 0.6, 0.6)
		sc = math.floor(sc / 10)
		len = len - 1
	end
	...
	if sc == 0 then
		love.graphics.drawq(image,quad_num[0], 230,gameheight/3 + 64, 0, 0.6, 0.6)
	end
	while sc ~= 0 do
		love.graphics.drawq(image,quad_num[sc % 10], 230 + len * 12, gameheight/3 + 64, 0 , 0.6, 0.6)
		sc = math.floor(sc / 10)
		len = len - 1
	end
